...
object JaBeJaProtocolNoStats {
  private[JaBeJaProtocolNoStats] implicit val accumSeq = new AccumulatorParam[Seq[(String, String)]] {
  ...
  private[JaBeJaProtocolNoStats] implicit val accumSet = new AccumulatorParam[Set[Int]] {
 ...
 * @param step_param
 */
class JaBeJaProtocolNoStats(random_seed: Long,
                     alfa_param: Double = 2.0,
                     val TStart: Double = 2.0,
                     delta: Double = 0.003,
                     randomCacheViewSize_param: Int,
                     tmanCacheViewSize_param: Int,
                     partnerPickOrder_param: PartnerPickOrderNoStats.Value = PartnerPickOrderNoStats.NEIGH_TMAN_RAND,
  ...
  private var partnerPickOrder: PartnerPickOrderNoStats.Value = _
...
//    bestMinCutList = sc.accumulator(Seq[(String, String)]())(JaBeJaProtocolNoStats.accumSeq)
...
//    colorSet = sc.accumulator[Set[Int]](Set[Int]())(JaBeJaProtocolNoStats.accumSet)
...
//    logDebug(strBuilder.toString())
  ...
  private def exchangeColor[K: ClassTag](self: JaBeJaVertexContextNoStats[K],
                                         partner: SendableJaBeJaVertexContextNoStats[K]): Unit = {
    val temp = partner.color
    partner.color = self.color
    self.color = temp
  }

  private def updateTMANView[K](self: JaBeJaVertexContextNoStats[K], other: SendableJaBeJaVertexContextNoStats[K]): Unit = {
  ...
  def compute[K: ClassTag](self: ProtocolVertexContext[K],
                           ...
                           aggregator: Option[JaBeJaProtocolNoStats#aggregatorType],
                           superstep: Int)
  : (Boolean, Seq[_ <: Message[K]], Seq[RequestVertexContextMessage[K]]) = {
    val cxtJaBeJa = self.asInstanceOf[JaBeJaVertexContextNoStats[K]]
...
//    }
    ...
    val (answerSwapRequestMsgs, swapRequestMsgs) = messages.partition {
      case msg: JaBeJaMessageNoStats[K] if msg.header != HeaderNoStats.SWAP_REQUEST => true
      case msg => false
    }
...
//      case msg: JaBeJaMessageNoStats[K] => msg.header == HeaderNoStats.ACK
//      case _ => false
//    }).size <= 1)
    ...
    def processMessages(messages: Seq[Message[K]]): Unit = {
      ...
      messages.foreach {
        case jabejaMsg: JaBeJaMessageNoStats[K] =>
...
//                case SWAP_TYPE_NoStats.TMAN => ackTMAN += 1
//                case SWAP_TYPE_NoStats.NEIGHBOURS => ackNeighbours += 1
//                case SWAP_TYPE_NoStats.RANDOM => ackRandom += 1
//              }
            case HeaderNoStats.NACK =>
              cxtJaBeJa.sentSwapRequest = false // Not engaged anymore

              // Update Statistics
//              cxtJaBeJa.lastSwapRequestType match {
//                case SWAP_TYPE_NoStats.TMAN => nackTMAN += 1
//                case SWAP_TYPE_NoStats.NEIGHBOURS => nackNeighbours += 1
//                case SWAP_TYPE_NoStats.RANDOM => nackRandom += 1
//              }
            case HeaderNoStats.SWAP_REQUEST =>
              if (!cxtJaBeJa.sentSwapRequest && !swappedColorFlag && cxtJaBeJa.evaluate(jabejaMsg.sourceCxt, alfa, Tr) > 0) {
                newMsgs += new JaBeJaMessageNoStats[K](cxtJaBeJa, name, jabejaMsg.sourceId, name, HeaderNoStats.ACK) // ACK MESSAGE
                exchangeColor(cxtJaBeJa, jabejaMsg.sourceCxt)
                ...
                newMsgs += new JaBeJaMessageNoStats[K](cxtJaBeJa, name, jabejaMsg.sourceId, name, HeaderNoStats.NACK) // NACK Message
              }
          }
      ...
      if (trySwapRequest && !cxtJaBeJa.sentSwapRequest) {
        ...
        val neighbours = () => {
          ...
          if (neighbourCandidateId != null) {
            ...
            // Statistics
            cxtJaBeJa.lastSwapRequestType = SWAP_TYPE_NoStats.NEIGHBOURS
//            swapRequestNeighbours += 1
        ...
        val tman = () => {
          ...
          if (!cxtJaBeJa.tmanCacheView.isEmpty) {
            val tmanCacheView = responseProtocolCxtMsgs.filter(msg => cxtJaBeJa.tmanCacheView.exists(id => id == msg.sourceId)).map(msg => (msg.sourceId, msg.cxt.asInstanceOf[SendableJaBeJaVertexContextNoStats[K]])).toArray
          ...
          if (partner != null) {
            // Statistics
            cxtJaBeJa.lastSwapRequestType = SWAP_TYPE_NoStats.TMAN
//            swapRequestTMAN += 1
          }
        }
        ...
        val random = () => {
          ...
          if (!cxtJaBeJa.randomCacheView.isEmpty) {
            val randomCacheView = responseProtocolCxtMsgs.filter(msg => cxtJaBeJa.randomCacheView.exists(id => id == msg.sourceId)).map(msg => (msg.sourceId, msg.cxt.asInstanceOf[SendableJaBeJaVertexContextNoStats[K]])).toArray
          ...
          if (partner != null) {
            // Statistics
            cxtJaBeJa.lastSwapRequestType = SWAP_TYPE_NoStats.RANDOM
//            swapRequestRandom += 1
          }
        }
        ...
        partnerPickOrder match {
          case PartnerPickOrderNoStats.NEIGH_TMAN_RAND => pickOrder = Array(neighbours, tman, random);
          case PartnerPickOrderNoStats.TMAN_NEIGH_RAND => pickOrder = Array(tman, neighbours, random);
          case PartnerPickOrderNoStats.RAND_TMAN_NEIGH => pickOrder = Array(random, tman, neighbours);
        }
...
//        triedSwapRequest += 1
      ...
      if (trySwapRequest && !cxtJaBeJa.sentSwapRequest) {
        ...
        if (partner != null) {
          newMsgs += new JaBeJaMessageNoStats[K](cxtJaBeJa, name, partner, name, HeaderNoStats.SWAP_REQUEST) // SWAP_REQUEST MESSAGE
        }
...
//      m => m match {
//        case jabejaMsg: JaBeJaMessageNoStats[K] if jabejaMsg.header == HeaderNoStats.ACK => true
//        case _ => false
//      }) <= 1
//    )
...
//        reqCxtGeneral += neighboursReqMsgs.size
  ...
  def nextTmanCacheChunk[K](cxtJaBeJa: JaBeJaVertexContextNoStats[K]): TraversableOnce[RequestVertexContextMessage[K]] = {
  ...
  def nextRandomCacheChunk[K](cxtJaBeJa: JaBeJaVertexContextNoStats[K]): TraversableOnce[RequestVertexContextMessage[K]] = {
   ...
   * @return
   */
  private def refreshMessages[K: ClassTag](cxtJaBeJa: JaBeJaVertexContextNoStats[K])
   ...
   */
  private def searchNeighbourPartnerCandidate[K](cxtJaBeJa: JaBeJaVertexContextNoStats[K]): K = {
   ...
   * @tparam K
   */
  private def updateView[K](cxtJaBeJa: JaBeJaVertexContextNoStats[K],
                            respMsgs: Seq[ResponseVertexContextMessage[K]],
                            bootstrapPhase: Boolean
                             ): Unit = {
    respMsgs.foreach {
      msg =>
        val otherCxt = msg.cxt.asInstanceOf[SendableJaBeJaVertexContextNoStats[K]]
   ...
   */
  private def updateView[K](cxtJaBeJa: JaBeJaVertexContextNoStats[K],
                            freshCxt: SendableJaBeJaVertexContextNoStats[K]): Unit = {
  ...
  : (Seq[_ <: Message[K]], Seq[RequestVertexContextMessage[K]]) =
    (new Array[Message[K]](0), refreshMessages(context.asInstanceOf[JaBeJaVertexContextNoStats[K]]))
...
//    colorSet += Set(data(0).hashCode()) // Add color to the set such a way it' possible to count how many colors occur
    ...
    val cxt = new JaBeJaVertexContextNoStats[K](data(0).asInstanceOf[Int])
    cxt.neighbourView = data.drop(1).map(id => (id.asInstanceOf[K], null).asInstanceOf[(K, SendableJaBeJaVertexContextNoStats[K])])
...
//    reqCxtDuplicated = sc.accumulator(0)(IntAccumulatorParam)
  ...
  def afterSuperstep(sc: SparkContext, superstep: Int, property : TelosPropertiesImmutable): Unit = {
    if (superstep > startStep + 1 * step) {
      ...
      val printFile = new FileWriter( "mincutUnstable.txt", true )
        
		printFile.write(property.dataset+","+property.jabejaPartition+","+property.jabejaUseTMan+","+superstep+","+(unstableMincut.value*0.5)+ "\n" )
...
//        }
        ...
        if (bestMinCutValue > currentMinCut) {
...
//        mincutHistory += currentMinCut
		...
		printFile.write(property.dataset+","+property.jabejaPartition+","+property.jabejaUseTMan+","+superstep+","+currentMinCut+ "\n" )
...
class JaBeJaVertexContextNoStats[K](color_param: Int) extends ProtocolVertexContext[K] {
   ...
   * Neighbours View
   */
  var neighbourView: Array[(K, SendableJaBeJaVertexContextNoStats[K])] = _
   ...
   * Statistic variable
   */
  var lastSwapRequestType: SWAP_TYPE_NoStats.Value = _

  /**
   * Clear the context remaining only its data
   * @return
   */
  override def sendable(): SendableJaBeJaVertexContextNoStats[K] = {
    val newNeighbourView: Array[(K, SendableJaBeJaVertexContextNoStats[K])] = neighbourView.map {
      case v if v._2 == null => v
      case v if v._2 != null =>
        val vDolly = new SendableJaBeJaVertexContextNoStats[K](v._2)
        (v._1, v._2.fixSendable(vDolly).asInstanceOf[SendableJaBeJaVertexContextNoStats[K]])
    }

    val dolly = new SendableJaBeJaVertexContextNoStats[K](this.color, newNeighbourView)
    super.fixSendable(dolly).asInstanceOf[SendableJaBeJaVertexContextNoStats[K]]
  }
  ...
  def findPartner(view: Array[(K, SendableJaBeJaVertexContextNoStats[K])], alfa: Double, Tr: Double, nCandidates: Int = 1): Array[SendableJaBeJaVertexContextNoStats[K]] = {
    val (highest, bestPartner) = view.foldLeft[(Double, Array[SendableJaBeJaVertexContextNoStats[K]])]((0.0, Array())) {
      case (prec, elem) =>
        val q = elem._2
        val highest = prec._1
  ...
  def findPartner(alfa: Double, Tr: Double, nCandidates: Int): Array[SendableJaBeJaVertexContextNoStats[K]] = {
    findPartner(neighbourView, alfa, Tr, nCandidates)
  }

  def evaluate(q: SendableJaBeJaVertexContextNoStats[K], alfa: Double, Tr: Double): Double = evaluate(q, alfa, Tr, neighbourView.exists(link => link._1 == q.getId()))

  def evaluate(q: SendableJaBeJaVertexContextNoStats[K], alfa: Double, Tr: Double, isNeighbour: Boolean): Double = {
 ...
 * @tparam K
 */
class SendableJaBeJaVertexContextNoStats[K](var color: Int,
                                     var neighbourView: Array[(K, SendableJaBeJaVertexContextNoStats[K])])
  extends ProtocolVertexContext[K] {

  def this(cxt: JaBeJaVertexContextNoStats[K]) {
    this(cxt.color, null)
  }

  def this(cxt: SendableJaBeJaVertexContextNoStats[K]) {
    this(cxt.color, null)
  }

  /**
   * The same as [[JaBeJaVertexContextNoStats.getDegree]]
   * @param color
   * @return
   */
  def getDegree(color: Int): Int = {
...
object HeaderNoStats extends Enumeration with Serializable {
 ...
 */
object SWAP_TYPE_NoStats extends Enumeration with Serializable {
  val NEIGHBOURS, RANDOM, TMAN = Value
}

object PartnerPickOrderNoStats extends Enumeration with Serializable {
  val NEIGH_TMAN_RAND, TMAN_NEIGH_RAND, RAND_TMAN_NEIGH = Value
}

class JaBeJaMessageNoStats[K](@transient source: JaBeJaVertexContextNoStats[K],
                       val sourceProtocol: String,
                       val targetId: K,
                       val targetProtocol: String,
                       header_param: HeaderNoStats.Value)
  extends Message[K] {

  override val sourceId: K = source.getId

  val header = header_param
  val sourceCxt: SendableJaBeJaVertexContextNoStats[K] = source.sendable() // Pack the sendable version!
}
